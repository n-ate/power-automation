...
namespace PowerAutomation.Models
{
    ...
    /// </summary>
    public class CompositeProcedure : IProcedure
    {
        private IProcedure[] _procedures = [];
        ...
        private void ThrowIfCircularReference(IProcedure[] procedures, string pedigree = "")
        {
            for (var i = 0; i < procedures.Length; i++)
            {
                ...
                if (procedure is CompositeProcedure p) ThrowIfCircularReference(p.Procedures, $"[{i}]"); //search recursively
            }
        }
    }
}