...
using PowerAutomation.Controls.Procedures;

namespace PowerAutomation.Controls
{
    public partial class WorkspaceViewerWidget : Widget, IViewControl<Workspace>
    {
        ...
        {
            var widget = new DetectionsWidget(Model.Application, Model.Detections);
            NavigateForward(widget);
        }

        private void ProceduresButton_Click(object sender, EventArgs e)
        {
            var widget = new ProceduresWidget(Model.Detections, Model.Procedures);
            NavigateForward(widget);
        }
